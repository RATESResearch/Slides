...
The slideshow directive collects all slides of your project and preps them
"""
...
class slideshow(nodes.General, nodes.Element):
    pass
...
class Slide(BaseAdmonition, SphinxDirective):
    ...
    def run(self) -> List[Node]:
        ...
        if isinstance(slide, nodes.system_message):
            ...
            # sliname = os.path.join('src', slide['docname'] + '-slides.rst')
            # print("Appending to file", sliname)
            # with open(sliname, "a") as f:
            #     f.write(".. Appending today \n")
            #     f.write('\n')
            #     f.write(title)
            #     f.write("-"*len(title))
            #     for line in self.content:
    ...
    def process_doc(self, env: BuildEnvironment, docname: str,
        ...
        for slide in document.findall(slide_node):
            ...
            if env.config.slide_emit_warnings:
                logger.warning(__("TODO entry found: %s"), slide[1].astext(),
                               location=slide)
...
class SlideShow(SphinxDirective):
    """
    A show of all slide entries.
    """
    ...
    def run(self) -> List[Node]:
        # Simply insert an empty slideshow node which will be replaced later
        # when process_slide_nodes is called
        return [slideshow('')]
...
class SlideShowProcessor:
    ...
    def process(self, doctree: nodes.document, docname: str) -> None:
        slides: List[slide_node] = sum(self.domain.slides.values(), [])
        for node in list(doctree.findall(slideshow)):
            if not self.config.slide_include_slides:
                node.parent.remove(node)
                continue
...
def setup(app: Sphinx) -> Dict[str, Any]:
    ...
    app.add_directive('slideshow', SlideShow)
    app.add_domain(SlideDomain)
    app.connect('doctree-resolved', SlideListProcessor)
    app.connect('doctree-resolved', SlideShowProcessor)
    return {
